...
import com.aming.orm.spi.utils.AReflectionUtil;
...
public class AExample {
     ...
     * @param entityClass
     */
    public AExample(Class<?> entityClass) {
     ...
     */
    public AExample(Class<?> entityClass, boolean exists) {
     ...
     */
    public AExample(Class<?> entityClass, boolean exists, boolean notNull) {
        this.exists = exists;
        this.notNull = notNull;
        oredCriteria = new ArrayList<Criteria>();
        this.entityClass = entityClass;
        table = AEntityHelper.getEntityTable(entityClass);
     ...
     * @param notNull - true时，如果值为空，就会抛出异常，false时，如果为空就不使用该字段的条件
     */
    public AExample(boolean exists, boolean notNull) {
        this.exists = exists;
        this.notNull = notNull;
        oredCriteria = new ArrayList<Criteria>();
        // 之类注意要填充这三个参数
//        table = GwEntityHelper.getEntityTable(entityClass);
//        tableName = table.getName();
//        //根据李领北建议修改#159
//        propertyMap = table.getPropertyMap();
    }
    ...
    private AExample(Builder builder) {
     ...
     * @return
     */
    public AExample excludeProperties(String... properties) {
        if (properties != null && properties.length > 0) {
            ...
            for (String property : properties) {
                if (propertyMap.containsKey(property)) {
                    this.excludeColumns.add(propertyMap.get(property).getSelectColumn());
                } else {
                    throw new RuntimeException("类 " + entityClass.getSimpleName() + " 不包含属性 \'" + property + "\'，或该属性被@Transient注释！");
                }
            }
        }
        return this;
     ...
     * @return
     */
    public AExample selectProperties(String... properties) {
        if (properties != null && properties.length > 0) {
            ...
            for (String property : properties) {
                if (propertyMap.containsKey(property)) {
                    this.selectColumns.add(propertyMap.get(property).getSelectColumn());
                } else {
                    throw new RuntimeException("类 " + entityClass.getSimpleName() + " 不包含属性 \'" + property + "\'，或该属性被@Transient注释！");
                }
            }
    ...
    public static class OrderBy {
        //属性和列对应
        protected Map<String, AEntityColumn> propertyMap;
        private AExample aExample;
        private Boolean isProperty;

        public OrderBy(AExample aExample, Map<String, AEntityColumn> propertyMap) {
            this.aExample = aExample;
            this.propertyMap = propertyMap;
        }

        private String property(String property) {
            if (StringUtils.isEmpty(property) || StringUtils.isEmpty(property.trim())) {
                throw new RuntimeException("接收的property为空！");
            }
        ...
        public OrderBy orderBy(String property) {
            ...
            if (!StringUtils.isEmpty(aExample.getOrderByClause())) {
                aExample.setOrderByClause(aExample.getOrderByClause() + "," + column);
            } else {
                aExample.setOrderByClause(column);
            }
            isProperty = true;
            return this;
        }
        ...
        public OrderBy desc() {
            if (isProperty) {
                aExample.setOrderByClause(aExample.getOrderByClause() + " DESC");
                isProperty = false;
            }
            return this;
        }
        ...
        public OrderBy asc() {
            if (isProperty) {
                aExample.setOrderByClause(aExample.getOrderByClause() + " ASC");
                isProperty = false;
            }
            return this;
        }
    ...
    protected abstract static class GeneratedCriteria {
        ...
        protected GeneratedCriteria(Map<String, AEntityColumn> propertyMap, boolean exists, boolean notNull) {
            super();
            this.exists = exists;
            this.notNull = notNull;
        ...
        public Criteria andEqualTo(Object param) {
            Map<String, Field> fieldMap = AReflectionUtil.getFieldMap(param.getClass());
            Set<String> properties = fieldMap.keySet();
            for (String property : properties) {
                //属性和列对应Map中有此属性
        ...
        public Criteria andAllEqualTo(Object param) {
            if (param instanceof Map) {
                ...
                for (Object key : set) {
                    ...
                    if (propertyMap.get(property) != null) {
                        Object value = ((Map<?, ?>) param).get(key);
                        //属性值不为空
                        if (value != null) {
                            andEqualTo(property, value);
                        } else {
            ...
            List<AEntityField> fields = AFieldHelper.getFields(param.getClass());
            for (AEntityField field : fields) {
                String property = field.getName();
                //属性和列对应Map中有此属性
                if (propertyMap.get(property) != null) {
                    Object value = AReflectionUtil.getFieldValue(param, property);
                    //属性值不为空
                    if (value != null) {
                        andEqualTo(property, value);
                    } else {
    ...
    public static class Criteria extends GeneratedCriteria {
        ...
        protected Criteria(Map<String, AEntityColumn> propertyMap, boolean exists, boolean notNull) {
    ...
    public static class Builder {
        ...
        private List<ASqls.Criteria> sqlsCriteria;
        //动态表名
        private List<AExample.Criteria> exampleCriterias;
        //动态表名
        ...
        public Builder(Class<?> entityClass, boolean exists, boolean notNull) {
            ...
            this.table = AEntityHelper.getEntityTable(entityClass);
            this.propertyMap = table.getPropertyMap();
            this.sqlsCriteria = new ArrayList<ASqls.Criteria>(2);
        }
        ...
        public Builder where(ASqls aSqls) {
            ASqls.Criteria criteria = aSqls.getCriteria();
            criteria.setAndOr("and");
...
//        }
        ...
        public Builder andWhere(ASqls aSqls) {
            ASqls.Criteria criteria = aSqls.getCriteria();
            criteria.setAndOr("and");
            this.sqlsCriteria.add(criteria);
            return this;
...
//        }
        ...
        public Builder orWhere(ASqls aSqls) {
            ASqls.Criteria criteria = aSqls.getCriteria();
            criteria.setAndOr("or");
            this.sqlsCriteria.add(criteria);
            return this;
...
//        }
        ...
        public AExample build() {
            this.exampleCriterias = new ArrayList<Criteria>();
            for (ASqls.Criteria criteria : sqlsCriteria) {
                AExample.Criteria exampleCriteria = new AExample.Criteria(this.propertyMap, this.exists, this.notNull);
                exampleCriteria.setAndOr(criteria.getAndOr());
                for (ASqls.Criterion criterion : criteria.getCriterions()) {
            ...
            return new AExample(this);
        }

        private void transformCriterion(AExample.Criteria exampleCriteria, String condition, String property, Object[] values, String andOr) {
            if (values.length == 0) {
                if ("and".equals(andOr)) {
...
//    }
    ...
    public Set<String> getSelectColumns() {
        if (selectColumns != null && selectColumns.size() > 0) {
            ...
            Collection<AEntityColumn> aEntityColumns = propertyMap.values();
            selectColumns = new LinkedHashSet<String>(aEntityColumns.size() - excludeColumns.size());
            for (AEntityColumn column : aEntityColumns) {
